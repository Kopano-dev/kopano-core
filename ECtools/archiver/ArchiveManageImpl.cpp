...
HRESULT ArchiveManageImpl::AttachTo(LPMDB lpArchiveStore, const tstring &strFoldername, const char *lpszArchiveServer, const abentryid_t &sUserEntryId, unsigned ulFlags, AttachType attachType)
{
	...
	if (bEqual) {
		m_lpLogger->Log(EC_LOGLEVEL_INFO, "User and archive store are the same.");
		return MAPI_E_INVALID_PARAMETER;
	}
	...
	if (std::any_of(lstArchives.cbegin(), lstArchives.cend(), id_match)) {
		m_lpLogger->logf(EC_LOGLEVEL_INFO, "An archive for this \"" TSTRING_PRINTF "\" is already present in this store.", m_strUser.c_str());
	...
	if (aType == UndefArchive) {
		...
		m_lpLogger->Log(EC_LOGLEVEL_INFO, "Attempted to create an archive folder in an archive store that has an archive in its root.");
		return MAPI_E_COLLISION;
	} else if (aType == MultiArchive && strFoldername.empty()) {
		m_lpLogger->Log(EC_LOGLEVEL_INFO, "Attempted to create an archive in the root of an archive store that has archive folders.");
		return MAPI_E_COLLISION;
	}
	...
	if (hr == MAPI_E_NOT_FOUND) {
		...
		if (hr == hrSuccess)
			m_lpLogger->logf(EC_LOGLEVEL_INFO, "Archive is already used by \"" TSTRING_PRINTF "\" (" TSTRING_PRINTF ").", strUser.c_str(), strFullname.c_str());
		else
			m_lpLogger->logf(EC_LOGLEVEL_INFO, "Archive is already used (user entry: %s).", sAttachedUserEntryId.tostring().c_str());
		return MAPI_E_COLLISION;
	...
	if (hr != hrSuccess) {
		m_lpLogger->logf(EC_LOGLEVEL_INFO, "Failed to set archive type to %d: %s (%x)",
	...
	if (hr != hrSuccess)
		m_lpLogger->logf(EC_LOGLEVEL_INFO, "Successfully attached \"" TSTRING_PRINTF "\" in \"Unknown\" for user \"" TSTRING_PRINTF "\".", strFoldername.empty() ? KC_T("Root") : strFoldername.c_str(), m_strUser.c_str());
	else
		m_lpLogger->logf(EC_LOGLEVEL_INFO, "Successfully attached \"" TSTRING_PRINTF "\" in \"" TSTRING_PRINTF "\" for user \"" TSTRING_PRINTF "\"'.", strFoldername.empty() ? KC_T("Root") : strFoldername.c_str(), ptrArchiveName->Value.LPSZ, m_strUser.c_str());
	return hrSuccess;
}
...
eResult ArchiveManageImpl::DetachFrom(const char *lpszArchiveServer, const TCHAR *lpszArchive, const TCHAR *lpszFolder)
{
	...
	if (iArchive == lstArchives.end()) {
		m_lpLogger->logf(EC_LOGLEVEL_INFO, "\"" TSTRING_PRINTF "\" has no archive on \"" TSTRING_PRINTF "\"", m_strUser.c_str(), lpszArchive);
	...
	if (lpszFolder == NULL) {
		...
		if (std::any_of(iNextArchive, lstArchives.end(), StoreCompare(ptrArchiveStoreEntryId->Value.bin))) {
			m_lpLogger->logf(EC_LOGLEVEL_INFO, "\"" TSTRING_PRINTF "\" has multiple archives on \"" TSTRING_PRINTF "\"", m_strUser.c_str(), lpszArchive);
	...
	if (lpszFolder) {
		...
		if (iArchive == lstArchives.end()) {
			m_lpLogger->logf(EC_LOGLEVEL_INFO, "\"" TSTRING_PRINTF "\" has no archive named \"" TSTRING_PRINTF "\" on \"" TSTRING_PRINTF "\"", m_strUser.c_str(), lpszFolder, lpszArchive);
			return MAPIErrorToArchiveError(MAPI_E_NOT_FOUND);
		}
	}
	...
	if (lpszFolder)
		m_lpLogger->logf(EC_LOGLEVEL_INFO, "Successfully detached \"" TSTRING_PRINTF "\" in \"" TSTRING_PRINTF "\" from \"" TSTRING_PRINTF "\".", lpszFolder, lpszArchive, m_strUser.c_str());
	else
		m_lpLogger->logf(EC_LOGLEVEL_INFO, "Successfully detached \"" TSTRING_PRINTF "\" from \"" TSTRING_PRINTF "\".", lpszArchive, m_strUser.c_str());
...
eResult ArchiveManageImpl::DetachFrom(unsigned int ulArchive)
{
	...
	if (ulArchive >= lstArchives.size()) {
		m_lpLogger->logf(EC_LOGLEVEL_INFO, "Archive %u does not exist.", ulArchive);
		return MAPIErrorToArchiveError(MAPI_E_NOT_FOUND);
	}
	...
	m_lpLogger->logf(EC_LOGLEVEL_INFO, "Successfully detached archive %u from \"" TSTRING_PRINTF "\".", ulArchive, m_strUser.c_str());
	return MAPIErrorToArchiveError(hrSuccess);
}
