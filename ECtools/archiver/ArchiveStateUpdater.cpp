    ...
    const std::list<SObjectEntry> &lstArchives)
{
	...
	if (!userName.empty())
		m_lpLogger->logf(EC_LOGLEVEL_INFO, "Auto detached %u archive(s) from \"" TSTRING_PRINTF "\".", ulDetachCount, userName.c_str());
	else {
		tstring strUserName;
		if (m_ptrSession->GetUserInfo(userId, &strUserName, NULL) == hrSuccess)
			m_lpLogger->logf(EC_LOGLEVEL_INFO, "Auto detached %u archive(s) from \"" TSTRING_PRINTF "\".", ulDetachCount, strUserName.c_str());
		else
			m_lpLogger->logf(EC_LOGLEVEL_INFO, "Auto detached %u archive(s).", ulDetachCount);
	}
...
HRESULT ArchiveStateUpdater::AddCouplingBased(const tstring &userName, const std::list<tstring> &lstCouplings, unsigned int ulAttachFlags)
{
	...
	if (lpManage == NULL) {
		m_lpLogger->Log(EC_LOGLEVEL_INFO, "Unable to dynamic cast to ArchiveManageImpl pointer.");
