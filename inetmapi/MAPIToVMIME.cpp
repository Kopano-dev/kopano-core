    ...
    vmime::shared_ptr<vmime::message> *lpvmMessage, unsigned int flags)
{
			   ...
			   (strcasecmp(lpMsgClass->Value.lpszA, "IPM.Note.SMIME") == 0))
	{
		...
		lpszRawSMTP = std::make_unique<char[]>(static_cast<ULONG>(sStreamStat.cbSize.QuadPart) + 1);
	...
	*bestBody = plaintext;
	...
	try {
		...
		// else just plaintext
			...
			auto crlfconv = std::make_unique<char[]>(strBodyConverted.length() * 2 + 1);
			size_t outsize = 0;
    ...
    vmime::shared_ptr<vmime::header> vmHeader, unsigned int flags)
{
	...
	for (unsigned int i = 0; i < cNames; ++i) {
		...
		auto str = std::make_unique<char[]>(l);
