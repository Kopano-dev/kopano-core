...
ZEND_FUNCTION(mapi_table_findrow)
{
	...
	RETVAL_FALSE;
	MAPI_G(hr) = MAPI_E_INVALID_PARAMETER;
	if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "ra|ll", &res,
	    &restrictionArray, &bkOrigin, &ulFlags) == FAILURE)
		return;
...
ZEND_FUNCTION(mapi_table_freebookmark)
{
	...
	RETVAL_FALSE;
	MAPI_G(hr) = MAPI_E_INVALID_PARAMETER;
	if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "rl", &res, &lbookmark) == FAILURE)
		return;
...
ZEND_FUNCTION(mapi_zarafa_createuser)
{
	...
	zval			*res = NULL;
	LPMDB			lpMsgStore = NULL;
	char *lpszUsername = nullptr, *lpszFullname = nullptr;
	char *lpszEmail = nullptr, *lpszPassword = nullptr;
	php_stringsize_t ulUsernameLen = 0, ulFullname = 0, ulEmail = 0, ulPassword = 0;
	long ulIsNonactive = false, ulIsAdmin = false;
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "rssss|ll", &res,
							...
							&lpszEmail, &ulEmail,
							&ulIsNonactive, &ulIsAdmin) == FAILURE)
		return;
...
ZEND_FUNCTION(mapi_zarafa_setuser)
{
	...
	long ulIsNonactive = 0, ulIsAdmin = 0;
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "rsssssll", &res,
	    &lpUserId, &cbUserId, &lpszUsername, &ulUsername, &lpszFullname,
	    &ulFullname, &lpszEmail, &ulEmail, &lpszPassword, &ulPassword,
	    &ulIsNonactive, &ulIsAdmin) == FAILURE)
		return;
...
ZEND_FUNCTION(mapi_zarafa_createstore)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "rls", &res,
	    &ulStoreType, &lpUserId, &cbUserId) == FAILURE)
		return;
...
ZEND_FUNCTION(mapi_zarafa_creategroup)
{
	...
	MAPI_G(hr) = MAPI_E_INVALID_PARAMETER;
	if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "rs", &res,
	    &sGroup.lpszGroupname, &cbGroupname) == FAILURE)
		return;
...
ZEND_FUNCTION(mapi_zarafa_deletegroup)
{
	...
	MAPI_G(hr) = MAPI_E_INVALID_PARAMETER;
	if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "rs", &res,
	    &lpszGroupname, &cbGroupname) == FAILURE)
		return;
...
ZEND_FUNCTION(mapi_zarafa_addgroupmember)
{
	...
	// params
	zval 			*res = NULL;
	ENTRYID *lpGroupId = nullptr, *lpUserId = nullptr;
	...
	MAPI_G(hr) = MAPI_E_INVALID_PARAMETER;
	if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "rss", &res,
	    &lpGroupId, &cbGroupId, &lpUserId, &cbUserId) == FAILURE)
		return;
...
ZEND_FUNCTION(mapi_zarafa_setgroup)
{
	...
	MAPI_G(hr) = MAPI_E_INVALID_PARAMETER;
	if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "rss", &res,
	    &lpGroupId, &cbGroupId, &lpszGroupname, &cbGroupname) == FAILURE)
		return;
...
ZEND_FUNCTION(mapi_zarafa_add_company_remote_viewlist)
{
	...
	LOG_BEGIN();
	zval			*res = NULL;
	ENTRYID *lpSetCompanyId = nullptr, *lpCompanyId = nullptr;
...
ZEND_FUNCTION(mapi_zarafa_add_user_remote_adminlist)
{
	...
	LOG_BEGIN();
	zval			*res = NULL;
	ENTRYID *lpUserId = nullptr, *lpCompanyId = nullptr;
...
ZEND_FUNCTION(mapi_zarafa_add_quota_recipient)
{
	...
	LOG_BEGIN();
	zval			*res = NULL;
	ENTRYID *lpRecipientId = nullptr, *lpCompanyId = nullptr;
