...
exit:
	if (er != erSuccess)
		ec_log_err("SerializeObject obj %d failed: %s (%x)",
			ulObjId, GetMAPIErrorMessage(kcerr_to_mapierr(er)), er);
	return er;
}
    ...
    struct propValArray **lppPropValArray)
{
	...
	if (ulRealObjType == MAPI_MESSAGE || ulRealObjType == MAPI_ATTACH) {
		...
		if (ulRealObjType == MAPI_MESSAGE) {
			...
			auto strQuery = "UPDATE properties SET val_ulong = val_ulong "s + (fHasAttach ? " | 16 " : " & ~16") + " WHERE hierarchyid = " + stringify(ulObjId) + " AND tag = " + stringify(PROP_ID(PR_MESSAGE_FLAGS)) + " AND type = " + stringify(PROP_TYPE(PR_MESSAGE_FLAGS));
...
exit:
	if (er != erSuccess) {
		...
		ec_log_err("DeserializeObject failed: %s (%x)",
			GetMAPIErrorMessage(kcerr_to_mapierr(er, ~0U /* anything that yields UNKNOWN */)), er);
	}
	soap_del_PointerTopropValArray(&lpPropValArray);
	return er;
}
