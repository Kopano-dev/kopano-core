...
public:
	...
	KC_HIDDEN ECRESULT UpdateTables(ECKeyTable::UpdateType, unsigned int flags, unsigned int obj_id, const std::vector<unsigned int> &objects, unsigned int obj_type);
...
protected:
	...
	KC_HIDDEN ECRESULT UpdateSubscribedTables(ECKeyTable::UpdateType, const TABLESUBSCRIPTION &, const std::vector<unsigned int> &child_id);
