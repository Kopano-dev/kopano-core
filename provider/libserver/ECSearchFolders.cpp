    ...
    const struct searchCriteria *lpSearchCriteria)
{
	...
	g_lpSessionManager->g_lpStatsCollector->Increment(SCN_SEARCHFOLDER_COUNT);
	if (!bReStartSearch)
		return erSuccess;
...
ECRESULT ECSearchFolders::CancelSearchFolder(unsigned int ulStoreID, unsigned int ulFolderId)
{
	...
	g_lpSessionManager->g_lpStatsCollector->Increment(SCN_SEARCHFOLDER_COUNT, -1);
	l_sf.unlock();
	...
	for (const auto &folder : iterStore->second) {
		...
		do {
			...
			if (er == KCERR_DATABASE_ERROR) {
				...
				g_lpSessionManager->g_lpStatsCollector->Increment(SCN_SEARCHFOLDER_UPDATE_RETRY);
				continue;
		...
		if (ulAttempts == 0) {
			...
			g_lpSessionManager->g_lpStatsCollector->Increment(SCN_SEARCHFOLDER_UPDATE_FAIL);
		}
    }
 exit:
	l_sf.unlock();
...
void* ECSearchFolders::SearchThread(void *lpParam)
{
    ...
    delete ti;
	g_lpSessionManager->g_lpStatsCollector->Increment(SCN_SEARCHFOLDER_THREADS);
    // Start the search
    ...
    lpFolder = NULL;
	g_lpSessionManager->g_lpStatsCollector->Increment(SCN_SEARCHFOLDER_THREADS, -1);
    return NULL;
}
