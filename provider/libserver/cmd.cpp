...
exit:
	...
	if (lpecSession)
		lpecSession->unlock();
	if (er == erSuccess)
		g_lpSessionManager->m_stats->Increment(SCN_LOGIN_SSO);
...
SOAP_ENTRY_START(getPublicStore, lpsResponse->er, unsigned int ulFlags, struct getStoreResponse *lpsResponse)
{
	...
	if (sesmgr->IsDistributedSupported()) {
		...
		if (strcasecmp(strThisServer.c_str(), strStoreServer.c_str()) != 0 &&
			...
			g_lpSessionManager->m_stats->Increment(SCN_REDIRECT_COUNT, 1);
    ...
    struct loadObjectResponse *lpsLoadObjectResponse)
{
	...
	g_lpSessionManager->m_stats->Increment(SCN_DATABASE_MWOPS);
}
SOAP_ENTRY_END()
    ...
    struct loadObjectResponse *lpsLoadObjectResponse)
{
	...
	g_lpSessionManager->m_stats->Increment(SCN_DATABASE_MROPS);
}
SOAP_ENTRY_END()
    ...
    exportMessageChangesAsStreamResponse *lpsResponse)
{
	...
	g_lpSessionManager->m_stats->Increment(SCN_DATABASE_MROPS, static_cast<int>(ulObjCnt));
	return erSuccess;
}
SOAP_ENTRY_END()
