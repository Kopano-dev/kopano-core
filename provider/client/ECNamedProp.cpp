...
static const struct {
	...
	{{0x62002, 0x0, 0x0, {0xC0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x46}}, 0x8200, 0x826F, 0x8000}, /* PSETID_Appointment */
	{{0x62003, 0x0, 0x0, {0xC0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x46}}, 0x8100, 0x813F, 0x8070}, /* PSETID_Task */
	{{0x62004, 0x0, 0x0, {0xC0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x46}}, 0x8000, 0x80EF, 0x80B0}, /* PSETID_Address */
	{{0x62008, 0x0, 0x0, {0xC0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x46}}, 0x8500, 0x85FF, 0x81A0}, /* PSETID_Common */
	{{0x6200A, 0x0, 0x0, {0xC0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x46}}, 0x8700, 0x871F, 0x82A0}, /* PSETID_Log */
	{{0x6200B, 0x0, 0x0, {0xC0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x46}}, 0x8800, 0x881F, 0x82C0},
	{{0x6200E, 0x0, 0x0, {0xC0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x46}}, 0x8B00, 0x8B1F, 0x82E0}, /* PSETID_Note */
	...
	{{0x6ED8DA90, 0x450B, 0x101B, {0x98, 0xDA, 0x00, 0xAA, 0x00, 0x3F, 0x13, 0x05}} , 0x0000, 0x003F, 0x8340}, /* PSETID_Meeting */
	{{0x63aed8c8, 0x4049, 0x4b75, {0xbc, 0x88, 0x96, 0xdf, 0x9d, 0x72, 0x3f, 0x2f}}, 0x0002, 0x0002, 0x8380}, /* PSETID_KC:dispidLicenseJSON */
};
    ...
    MAPINAMEID ***lpppPropNames)
{
	...
	memory_ptr<MAPINAMEID *> lppPropNames, lppResolved;
	unsigned int cResolved = 0, cUnresolved = 0;

	auto lpsPropTags = *lppPropTags;
	// Allocate space for properties
	auto hr = MAPIAllocateBuffer(sizeof(MAPINAMEID *) * lpsPropTags->cValues, &~lppPropNames);
	if (hr != hrSuccess)
		return hr;
	...
	memory_ptr<SPropTagArray> lpsUnresolved;
	hr = MAPIAllocateBuffer(CbNewSPropTagArray(lpsPropTags->cValues), &~lpsUnresolved);
	if (hr != hrSuccess)
		return hr;
    ...
    MAPINAMEID **lppPropNames, unsigned int ulFlags, SPropTagArray **lppPropTags)
{
	...
	auto hr = MAPIAllocateBuffer(CbNewSPropTagArray(cPropNames), &~lpsPropTagArray);
	if(hr != hrSuccess)
		return hr;
	...
	if(cUnresolved) {
		...
		memory_ptr<unsigned int> lpServerIDs;
...
HRESULT ECNamedProp::UpdateCache(ULONG ulId, MAPINAMEID *lpName)
{
	...
	memory_ptr<MAPINAMEID> lpNameCopy;
...
HRESULT ECNamedProp::HrCopyNameId(MAPINAMEID *lpSrc, MAPINAMEID **lppDst, void *lpBase)
{
	...
	auto hr = MAPIAllocateMore(sizeof(MAPINAMEID), lpBase, reinterpret_cast<void **>(&lpDst));
	if(hr != hrSuccess)
		return hr;

	lpDst->ulKind = lpSrc->ulKind;
	if(lpSrc->lpguid) {
		hr = MAPIAllocateMore(sizeof(GUID), lpBase != nullptr ? lpBase : lpDst,
	...
	case MNID_ID:
		...
		break;
	case MNID_STRING:
		hr = MAPIAllocateMore(wcslen(lpSrc->Kind.lpwstrName) * sizeof(wchar_t) + sizeof(wchar_t),
