...
HRESULT CopyMAPIPropValToSOAPPropVal(propVal *dp, const SPropValue *sp) {
	...
	case PT_MV_STRING8:
		...
		for (gsoap_size_t i = 0; i < dp->Value.mvszA.__size; ++i)
			dp->Value.mvszA.__ptr[i] = soap_strdup(
				nullptr, convert_to<utf8string>(sp->Value.MVszA.lppszA[i]).z_str());
		break;
	case PT_MV_UNICODE:
		dp->__union = SOAP_UNION_propValData_mvszA;
		dp->Value.mvszA.__size = sp->Value.MVszA.cValues;
		...
		for (gsoap_size_t i = 0; i < dp->Value.mvszA.__size; ++i)
			dp->Value.mvszA.__ptr[i] = soap_strdup(
				nullptr, convert_to<utf8string>(sp->Value.MVszW.lppszW[i]).z_str());
		break;
	case PT_MV_CLSID:
		dp->__union = SOAP_UNION_propValData_mvbin;
	...
	case PT_ACTIONS: {
		...
		for (unsigned int i = 0; i < lpSrcActions->cActions; ++i) {
			...
			case OP_DELEGATE: {
				...
				auto hr = CopyMAPIRowSetToSOAPRowSet(
					reinterpret_cast<const SRowSet *>(sa->lpadrlist), &da->act.adrlist);
				if(hr != hrSuccess)
					return hr;
				break;
			}
...
HRESULT CopySOAPPropValToMAPIPropVal(SPropValue *dp, const struct propVal *sp,
	...
	case PT_MV_STRING8: {
		...
		for (unsigned int i = 0; i < dp->Value.MVszA.cValues; ++i) {
			if (sp->Value.mvszA.__ptr[i] != NULL) {
				auto s = convert_to<std::string>(
					sp->Value.mvszA.__ptr[i], rawsize(sp->Value.mvszA.__ptr[i]), "UTF-8");
	...
	case PT_MV_UNICODE: {
		...
		for (unsigned int i = 0; i < dp->Value.MVszW.cValues; ++i) {
			...
			auto ws = convert_to<std::wstring>(
				sp->Value.mvszA.__ptr[i], rawsize(sp->Value.mvszA.__ptr[i]), "UTF-8");
	...
	case PT_SRESTRICTION: {
		...
		if (hr != hrSuccess)
			return hr;
		return CopySOAPRestrictionToMAPIRestriction(
			reinterpret_cast<SRestriction *>(dp->Value.lpszA), sp->Value.res, lpBase);
	}
	case PT_ACTIONS: {
			...
			case OP_DELEGATE:
				...
				for (gsoap_size_t j = 0; j < sa->act.adrlist->__size; ++j) {
					...
					hr = CopySOAPRowToMAPIRow(
						&sa->act.adrlist->__ptr[j],
						da->lpadrlist->aEntries[j].rgPropVals,
						lpBase);
					if (hr != hrSuccess)
						return hr;
				}
...
HRESULT CopySOAPRowToMAPIRow(const struct propValArray *lpsRowSrc, LPSPropValue lpsRowDst, void *lpBase) {
	for (gsoap_size_t j = 0; j < lpsRowSrc->__size; ++j) {
		// If all fails, get the actual data from the server
...
HRESULT CopyMAPIRowSetToSOAPRowSet(const SRowSet *lpRowSetSrc, struct rowSet **lppsRowSetDst) {
	auto lpsRowSetDst = soap_new_rowSet(nullptr);
	lpsRowSetDst->__ptr = nullptr;
...
HRESULT CopySOAPRowSetToMAPIRowSet(
	...
	for (lpRowSet->cRows = 0; lpRowSet->cRows < ulRows; ++lpRowSet->cRows) {
		...
		if (hr != hrSuccess)
			return hr;
		CopySOAPRowToMAPIRow(
			lpProvider,
			&lpsRowSetSrc->__ptr[i],
			lpRowSet->aRow[i].lpProps,
			reinterpret_cast<void **>(lpRowSet->aRow[i].lpProps),
			ulType);
	}
...
HRESULT CopySOAPRestrictionToMAPIRestriction(LPSRestriction lpDst, const struct restrictTable *lpSrc, void *lpBase) {
	if (lpSrc == nullptr)
		return MAPI_E_INVALID_PARAMETER;
	...
	case RES_OR: {
		...
		for (gsoap_size_t i = 0; i < lpSrc->lpOr->__size; ++i) {
			hr = CopySOAPRestrictionToMAPIRestriction(
				&lpDst->res.resOr.lpRes[i], lpSrc->lpOr->__ptr[i], lpBase);
	...
	case RES_AND: {
		...
		for (gsoap_size_t i = 0; i < lpSrc->lpAnd->__size; ++i) {
			hr = CopySOAPRestrictionToMAPIRestriction(
				&lpDst->res.resAnd.lpRes[i], lpSrc->lpAnd->__ptr[i], lpBase);
	...
	case RES_COMMENT: {
		...
		if (hr != hrSuccess)
			return hr;
		hr = CopySOAPRestrictionToMAPIRestriction(
			lpDst->res.resComment.lpRes, lpSrc->lpComment->lpResTable, lpBase);
		if (hr != hrSuccess)
			return hr;
		...
		for (gsoap_size_t i = 0; i < lpSrc->lpComment->sProps.__size; ++i) {
			hr = CopySOAPPropValToMAPIPropVal(
				&lpDst->res.resComment.lpProp[i], &lpSrc->lpComment->sProps.__ptr[i], lpBase);
			if (hr != hrSuccess)
				return hr;
		}
		break;
	...
	case RES_CONTENT: {
		...
		if(hr != hrSuccess)
			return hr;
		return CopySOAPPropValToMAPIPropVal(lpDst->res.resContent.lpProp, lpSrc->lpContent->lpProp, lpBase);
	}
	case RES_EXIST:
	...
	case RES_NOT: {
		...
		if (hr != hrSuccess)
			return hr;
		return CopySOAPRestrictionToMAPIRestriction(lpDst->res.resNot.lpRes, lpSrc->lpNot->lpNot, lpBase);
	}
	case RES_PROPERTY: {
		...
		return CopySOAPPropValToMAPIPropVal(lpDst->res.resProperty.lpProp, lpSrc->lpProp->lpProp, lpBase);
	}
	case RES_SIZE:
	...
	case RES_SUBRESTRICTION: {
		...
		if (hr != hrSuccess)
			return hr;
		return CopySOAPRestrictionToMAPIRestriction(
			lpDst->res.resSub.lpRes, lpSrc->lpSub->lpSubObject, lpBase);
	}
	default:
		return MAPI_E_INVALID_PARAMETER;
	}
...
HRESULT CopyMAPIRestrictionToSOAPRestriction(struct restrictTable **lppDst, const SRestriction *lpSrc) {
	...
	case RES_OR:
		...
		for (unsigned int i = 0; i < lpSrc->res.resOr.cRes; ++i) {
			hr = CopyMAPIRestrictionToSOAPRestriction(
				&(lpDst->lpOr->__ptr[i]), &lpSrc->res.resOr.lpRes[i]);
	...
	case RES_AND:
		...
		for (unsigned int i = 0; i < lpSrc->res.resAnd.cRes; ++i) {
			hr = CopyMAPIRestrictionToSOAPRestriction(
				&lpDst->lpAnd->__ptr[i], &lpSrc->res.resAnd.lpRes[i]);
	...
	case RES_COMMENT:
		...
		for (unsigned int i = 0; i < lpSrc->res.resComment.cValues; ++i) {
			hr = CopyMAPIPropValToSOAPPropVal(
				&lpDst->lpComment->sProps.__ptr[i], &lpSrc->res.resComment.lpProp[i]);
			if(hr != hrSuccess)
				return hr;
		}
	...
	case RES_CONTENT:
		...
		if( (PROP_TYPE(lpSrc->res.resContent.lpProp->ulPropTag) != PT_BINARY &&
			...
			(PROP_TYPE(lpSrc->res.resContent.lpProp->ulPropTag) == PT_BINARY && lpSrc->res.resContent.lpProp->Value.bin.cb >0 && lpSrc->res.resContent.lpProp->Value.bin.lpb == nullptr) ||
			(PROP_TYPE(lpSrc->res.resContent.lpProp->ulPropTag) == PT_STRING8 && lpSrc->res.resContent.lpProp->Value.lpszA == nullptr) ||
			(PROP_TYPE(lpSrc->res.resContent.lpProp->ulPropTag) == PT_UNICODE && lpSrc->res.resContent.lpProp->Value.lpszW == nullptr))
			return MAPI_E_INVALID_PARAMETER;
...
static HRESULT CopySOAPPropTagArrayToMAPIPropTagArray(
	const struct propTagArray *lpsPropTagArray, LPSPropTagArray *lppPropTagArray, void *lpBase) {
...
HRESULT Utf8ToTString(LPCSTR lpszUtf8, ULONG ulFlags, LPVOID lpBase, LPTSTR *lppszTString) {
	if (lpszUtf8 == nullptr || lppszTString == nullptr)
		return MAPI_E_INVALID_PARAMETER;

	std::string strDest = convert_to<std::string>(
		((ulFlags & MAPI_UNICODE) ? CHARSET_WCHAR : CHARSET_CHAR), lpszUtf8, rawsize(lpszUtf8), "UTF-8");
...
static HRESULT TStringToUtf8(struct soap *alloc, const TCHAR *lpszTstring, unsigned int ulFlags, char **lppszUtf8) {
	...
	if (ulFlags & MAPI_UNICODE)
		strDest = convert_to<std::string>(
			"UTF-8",
			reinterpret_cast<const wchar_t *>(lpszTstring),
			rawsize(reinterpret_cast<const wchar_t *>(lpszTstring)),
			CHARSET_WCHAR);
	else
		strDest = convert_to<std::string>(
			"UTF-8",
			reinterpret_cast<const char *>(lpszTstring),
			rawsize(reinterpret_cast<const char *>(lpszTstring)),
			CHARSET_CHAR);
...
HRESULT CopyABPropsFromSoap(
	const struct propmapPairArray *lpsoapPropmap,
	const struct propmapMVPairArray *lpsoapMVPropmap,
	SPROPMAP *lpPropmap,
	...
	if (lpsoapPropmap != nullptr) {
		...
		for (gsoap_size_t i = 0; i < lpsoapPropmap->__size; ++i) {
			...
			hr = Utf8ToTString(
				...
				ulConvFlags,
				lpBase,
				&lpPropmap->lpEntries[i].lpszValue);
			if (hr != hrSuccess)
				return hr;
		}
	}

	if (lpsoapMVPropmap != nullptr) {
		...
		for (gsoap_size_t i = 0; i < lpsoapMVPropmap->__size; ++i) {
			...
			for (gsoap_size_t j = 0; j < lpsoapMVPropmap->__ptr[i].sValues.__size; ++j) {
				hr = Utf8ToTString(
					...
					ulConvFlags,
					lpBase,
					&lpMVPropmap->lpEntries[i].lpszValues[j]);
				if (hr != hrSuccess)
					return hr;
			}
		}
...
HRESULT CopyABPropsToSoap(
	struct soap *alloc,
	const SPROPMAP *lpPropmap,
	...
	if (lpPropmap && lpPropmap->cEntries) {
		...
		for (gsoap_size_t i = 0; i < soapPropmap->__size; ++i) {
			...
			auto hr = TStringToUtf8(
				alloc,
				lpPropmap->lpEntries[i].lpszValue,
				ulConvFlags,
				&soapPropmap->__ptr[i].lpszValue);
			if (hr != hrSuccess)
				return hr;
		}
	}
		...
		for (gsoap_size_t i = 0; i < soapMVPropmap->__size; ++i) {
			...
			for (gsoap_size_t j = 0; j < soapMVPropmap->__ptr[i].sValues.__size; ++j) {
				auto hr = TStringToUtf8(
					...
					lpMVPropmap->lpEntries[i].lpszValues[j],
					ulConvFlags,
					&soapMVPropmap->__ptr[i].sValues.__ptr[j]);
				if (hr != hrSuccess)
					return hr;
			}
		}
...
static HRESULT SoapUserToUser(const struct user *lpUser, ECUSER *lpsUser, ULONG ulFlags, void *lpBase) {
...
HRESULT SoapUserArrayToUserArray(
	const struct userArray *lpUserArray, ULONG ulFlags, ULONG *lpcUsers, ECUSER **lppsUsers) {
...
HRESULT SoapUserToUser(const struct user *lpUser, ULONG ulFlags, ECUSER **lppsUser) {
	...
	if (hr != hrSuccess)
		return hr;
	hr = SoapUserToUser(lpUser, lpsUser, ulFlags, NULL);
	if (hr != hrSuccess)
		return hr;
...
static HRESULT SoapGroupToGroup(const struct group *lpGroup, ECGROUP *lpsGroup, ULONG ulFlags, void *lpBase) {
...
HRESULT SoapGroupArrayToGroupArray(
	const struct groupArray *lpGroupArray, ULONG ulFlags, ULONG *lpcGroups, ECGROUP **lppsGroups) {
...
HRESULT SoapGroupToGroup(const struct group *lpGroup, ULONG ulFlags, ECGROUP **lppsGroup) {
	...
	if (hr != hrSuccess)
		return hr;
	hr = SoapGroupToGroup(lpGroup, lpsGroup, ulFlags, NULL);
	if (hr != hrSuccess)
		return hr;
...
static HRESULT SoapCompanyToCompany(
	const struct company *lpCompany, ECCOMPANY *lpsCompany, ULONG ulFlags, void *lpBase) {
...
HRESULT SoapCompanyArrayToCompanyArray(
	const struct companyArray *lpCompanyArray, ULONG ulFlags,ULONG *lpcCompanies, ECCOMPANY **lppsCompanies) {
...
HRESULT SoapCompanyToCompany(const struct company *lpCompany, ULONG ulFlags, ECCOMPANY **lppsCompany) {
	...
	if (hr != hrSuccess)
		return hr;
	hr = SoapCompanyToCompany(lpCompany, lpsCompany, ulFlags, NULL);
	if (hr != hrSuccess)
		return hr;
...
HRESULT SvrNameListToSoapMvString8(
	struct soap *alloc, ECSVRNAMELIST *lpSvrNameList, unsigned int ulFlags, struct mv_string8 *&lpsSvrNameList) {
	...
	if (lpSvrNameList->cServers > 0) {
		...
		for (unsigned i = 0; i < lpSvrNameList->cServers; ++i) {
			auto hr = TStringToUtf8(
				alloc, lpSvrNameList->lpszaServer[i], ulFlags, &lpsSvrNameList->__ptr[i]);
			if (hr != hrSuccess)
				return hr;
		}
	}
...
HRESULT SoapServerListToServerList(
	const struct serverList *lpsServerList, ULONG ulFLags, ECSERVERLIST **lppServerList) {
	...
	for (gsoap_size_t i = 0; i < lpsServerList->__size; ++i) {
		...
		if (lpsServerList->__ptr[i].lpszName != NULL) {
			hr = Utf8ToTString(
				lpsServerList->__ptr[i].lpszName,
				ulFLags,
				lpServerList,
				&lpServerList->lpsaServer[i].lpszName);
			if (hr != hrSuccess)
				return hr;
		}
		...
		if (lpsServerList->__ptr[i].lpszFilePath != NULL) {
			hr = Utf8ToTString(lpsServerList->__ptr[i].lpszFilePath, ulFLags, lpServerList, &lpServerList->lpsaServer[i].lpszFilePath);
			if (hr != hrSuccess)
				return hr;
		}
		...
		if (lpsServerList->__ptr[i].lpszHttpPath != NULL) {
			hr = Utf8ToTString(
				lpsServerList->__ptr[i].lpszHttpPath,
				ulFLags,
				lpServerList,
				&lpServerList->lpsaServer[i].lpszHttpPath);
			if (hr != hrSuccess)
				return hr;
		}
		...
		if (lpsServerList->__ptr[i].lpszSslPath != NULL) {
			hr = Utf8ToTString(lpsServerList->__ptr[i].lpszSslPath,
				ulFLags,
				lpServerList,
				&lpServerList->lpsaServer[i].lpszSslPath);
			if (hr != hrSuccess)
				return hr;
		}
		...
		if (lpsServerList->__ptr[i].lpszPreferedPath != NULL) {
			hr = Utf8ToTString(
				lpsServerList->__ptr[i].lpszPreferedPath,
				ulFLags,
				lpServerList,
				&lpServerList->lpsaServer[i].lpszPreferedPath);
			if (hr != hrSuccess)
				return hr;
		}
	}
...
HRESULT WrapServerClientStoreEntry(
	const char *lpszServerName, const entryId *lpsStoreId, ULONG *lpcbStoreID, ENTRYID **lppStoreID) {
...
HRESULT UnWrapServerClientStoreEntry(
	ULONG cbWrapStoreID, const ENTRYID *lpWrapStoreID, ULONG *lpcbUnWrapStoreID, ENTRYID **lppUnWrapStoreID) {
...
HRESULT UnWrapServerClientABEntry(
	ULONG cbWrapABID, const ENTRYID *lpWrapABID, ULONG *lpcbUnWrapABID, ENTRYID **lppUnWrapABID) {
...
HRESULT CopySOAPNotificationToMAPINotification(
	void *lpProvider, const struct notification *lpSrc, NOTIFICATION **lppDst) {
...
HRESULT CopySOAPChangeNotificationToSyncState(const struct notification *lpSrc, SBinary **lppDst, void *lpBase) {
...
static HRESULT CopyMAPISourceKeyToSoapSourceKey(
...
HRESULT CopyICSChangeToSOAPSourceKeys(ULONG cbChanges, const ICSCHANGE *lpsChanges, sourceKeyPairArray **lppsSKPA) {
...
static HRESULT ConvertString8ToUnicode(LPSRestriction lpRestriction, void *base) {
	...
	case RES_COMMENT:
		...
		for (unsigned int i = 0; i < lpRestriction->res.resComment.cValues; ++i) {
			if (PROP_TYPE(lpRestriction->res.resComment.lpProp[i].ulPropTag) != PT_STRING8)
				continue;
			auto hr = ConvertString8ToUnicode(
				lpRestriction->res.resComment.lpProp[i].Value.lpszA,
				&lpRestriction->res.resComment.lpProp[i].Value.lpszW,
				base);
			if (hr != hrSuccess)
				return hr;
	...
	case RES_CONTENT: {
		if (PROP_TYPE(lpRestriction->res.resContent.ulPropTag) != PT_STRING8)
			break;
		auto hr = ConvertString8ToUnicode(
			lpRestriction->res.resContent.lpProp->Value.lpszA,
			&lpRestriction->res.resContent.lpProp->Value.lpszW,
			base);
		if (hr != hrSuccess)
			return hr;
	...
	case RES_PROPERTY: {
		if (PROP_TYPE(lpRestriction->res.resProperty.ulPropTag) != PT_STRING8)
			break;
		auto hr = ConvertString8ToUnicode(
			lpRestriction->res.resProperty.lpProp->Value.lpszA,
			&lpRestriction->res.resProperty.lpProp->Value.lpszW,
			base);
		if (hr != hrSuccess)
			return hr;
...
HRESULT ConvertString8ToUnicode(LPSRow lpRow, void *base) {
	if (lpRow == NULL)
		return hrSuccess;
	...
	for (ULONG c = 0; c < lpRow->cValues; ++c) {
		...
		if (PROP_TYPE(lpRow->lpProps[c].ulPropTag) == PT_SRESTRICTION) {
			hr = ConvertString8ToUnicode(
				reinterpret_cast<SRestriction *>(lpRow->lpProps[c].Value.lpszA),
				base != nullptr ? base : lpRow->lpProps);
		} else if (PROP_TYPE(lpRow->lpProps[c].ulPropTag) == PT_ACTIONS) {
			hr = ConvertString8ToUnicode(
				reinterpret_cast<ACTIONS *>(lpRow->lpProps[c].Value.lpszA),
				base != nullptr ? base : lpRow->lpProps);
			...
			hr = ConvertString8ToUnicode(
				lpRow->lpProps[c].Value.lpszA, &lpRow->lpProps[c].Value.lpszW, base);
			if (hr != hrSuccess)
				return hr;
...
HRESULT ConvertString8ToUnicode(LPSRowSet lpRowSet) {
	...
	for (ULONG c = 0; c < lpRowSet->cRows; ++c) {
		auto hr = ConvertString8ToUnicode(&lpRowSet->aRow[c], NULL);
		if (hr != hrSuccess)
			return hr;
	}
	return hrSuccess;
...
HRESULT convert_wsfolder_to_soapfolder(
	const std::vector<WSMAPIFolderOps::WSFolder> &source, std::vector<new_folder> &destination) {
...
HRESULT convert_soapfolders_to_wsfolder(
	const struct create_folders_response &source, std::vector<WSMAPIFolderOps::WSFolder> &destination) {
