    ...
    ULONG ulFlags)
{
	...
	scoped_rlock lock(m_hLock);
	HRESULT hr = MAPIAllocateBuffer(CbNewSPropTagArray(lpPropTagArray->cValues), &~lpsPropTags);
	if (hr != hrSuccess)
		return hr;
	...
	hr = MAPIAllocateBuffer(CbNewSPropTagArray(lpPropTagArray->cValues), &~m_lpSetColumns);
	if (hr != hrSuccess)
		return hr;
