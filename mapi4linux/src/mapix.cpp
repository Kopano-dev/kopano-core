    ...
    const TCHAR *lpszNewEntryTitle, ADRLIST *lpAdrList)
{
	...
	for (unsigned int i = 0; i < lpAdrList->cEntries; ++i) {
		...
		hr = MAPIAllocateMore(cbOneEntryID, lpNewProps, (void **)&lpNewProps[0].Value.bin.lpb);
		if (hr != hrSuccess)
			return hr;
    ...
    const SPropTagArray *lpPropTagArray, LPADRLIST lpRecipList)
{
	...
	for (unsigned int i = 0; i < lpRecipList->cEntries; ++i) {
		...
		hr = MAPIAllocateBuffer(sizeof(SPropValue) * lpPropTagArray->cValues,
		     (void **)&lpRecipList->aEntries[i].rgPropVals);
		if (hr != hrSuccess)
			return hr;
